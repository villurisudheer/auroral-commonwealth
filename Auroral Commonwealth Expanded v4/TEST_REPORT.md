...
## Expanded roster v4 verification

- 199 roster entries loaded with 199 unique IDs.
- Micronesia retained as the existing Federated States of Micronesia entry (`fsm`) rather than duplicated.
- Added Transnistria (`tns`) as a clearly labelled de facto/disputed entry.
- Added The Republic of Samratpur (`srp`), நாய் Country (`nai`), and Federation of Hyperpixel (`hpx`) as fictional entries.
- All 199 entries completed a 12-month simulation without NaN/Infinity.
- Country data, map UI, and main client syntax checks passed.
