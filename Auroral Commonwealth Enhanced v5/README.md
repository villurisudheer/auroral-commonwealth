# AURORAL COMMONWEALTH — Enhanced v5

## Expanded Historical Scenarios (v5)

The world roster now contains 202 selectable entries. In addition to the 195 sovereign-state templates, the special disputed/fictional entries, and Micronesia, v5 adds a **Historical** country category with:

- Union of Soviet Socialist Republics (USSR)
- WWII Peak Germany — neutral historical presentation with no extremist symbols
- Ottoman Empire

Historical starting values are deliberately simplified and balanced for gameplay. They are not intended as exact historical economic reconstructions. Historical entries coexist with modern countries as optional sandbox scenarios.


A browser-based country/economy strategy sandbox with single-player, online-ready multiplayer, and LAN multiplayer.

## World roster upgrade

- 199 world-roster entries are selectable: 195 sovereign states, Transnistria as a clearly labelled de facto/disputed entry, and three fictional nations.
- Country search and region filters make the larger roster manageable, including dedicated `Disputed / De facto` and `Fictional` filters.
- Strategic map uses stored latitude/longitude positions and includes a full country directory.
- Up to 20 human players can join a room; unclaimed countries remain available for diplomacy and future AI control.
- Country values are simplified for gameplay and are not real-world forecasts.


## Enhanced v5 interface features

- **Light / Dark / System appearance modes**. System mode follows the device preference.
- Appearance preference persists locally in the browser and applies to both single-player and multiplayer.
- **Quick theme cycle** button on the in-game command bar and `T` keyboard shortcut.
- **Comfortable / Compact interface density** for different screen sizes.
- **High-contrast mode** with stronger borders and selection states.
- Existing animation controls remain available and can be disabled for comfort/performance.
- **Configurable autosave frequency**: every 1, 3, 6, or 12 game months.
- New **in-game Settings** navigation page, so visual preferences can be changed without leaving a campaign.
- Keyboard shortcuts: `Space` pause/resume, `0` pause, `1`/`2`/`5` simulation speed, `T` theme, and `S` quick save in single player.
- Chart labels and grid lines automatically adapt to the active appearance mode.
- Includes the expanded 202-entry roster: modern sovereign states plus clearly labelled disputed/de facto, fictional, and historical scenario entries.

## What changed in v2

- Multiplayer rooms with six-character room codes.
- Up to 20 human players per room.
...
- Multiplayer is designed for small private rooms (maximum 20 players), not massive public servers.
